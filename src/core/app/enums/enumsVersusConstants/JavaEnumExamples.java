...
public class JavaEnumExamples {
     ...
     * If we look at these methods, we have two risks with using constants that are solved by the enum.
     * <p>
     * 1.We can pass any int constant to the simpleConstantsExample method but we can pass only fixed values
     * to simpleEnumExample, so it provides type safety.
     * 2.We can change the int constants value in ThreadStatesConstant class but the above program will not
     * throw any exception. Our program might not work as expected but if we change the enum constants,
